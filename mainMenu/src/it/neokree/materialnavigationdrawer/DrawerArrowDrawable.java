...
/**
 * A drawable that rotates between a
 * drawer icon and a back arrow based on parameter.
...
public class DrawerArrowDrawable extends Drawable {
    ...
    private final PathMeasure mearsureFirst;
    private final PathMeasure mearsureSecond;
    private final float lengthFirst;
    private final float lengthSecond;
    
    private JoinedPath(Path pathFirst,Path pathSecond){
        mearsureFirst = new PathMeasure(pathFirst,false);
        mearsureSecond = new PathMeasure(pathSecond,false);
        lengthFirst = mearsureFirst.getLength();
        lengthSecond = mearsureSecond.getLength();
    }
    /**
    ...
    private void getPointOnLine(float parameter,float[] coords){
        if (parameter <= .5f){
            ...
            parameter *= 2;
            mearsureFirst.getPosTan(lengthFirst * parameter, coords, null);
        } else {
            parameter -= .5f;
            parameter *= 2;
            mearsureSecond.getPosTan(lengthSecond * parameter, coords, null);
        }
    }
